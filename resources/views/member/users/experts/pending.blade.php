...
@section('content')
    <div class="dashboard-box main-box-in-row">
        <div class="headline">
            <h3><i class="icon-feather-bar-chart-2"></i> Freelancers - Pending Approval</h3>
        </div>
        <div class="contentdashboard-box main-box-in-row table-responsive" style="overflow-x:auto; padding: 10px;">
            @if($experts->count() < 1)
                <div class="row">
                    <div class="col-md-12">
                        <p style="padding: 20px;">Bravo! No pending experts in the system.</p>
                    </div>
                </div>
            @else
                        ...
                        <tbody>
                            @foreach($experts as $expert)
                                <tr>
                                    ...
                                    <td>
                                        {{ \Carbon\Carbon::parse($expert->created_at)->isoFormat('MMM Do, h:mm A') }}
                                    </td>
                                    <td>
                                        <button class="approve btn btn-success" style="color: #ffffff;" href="#" data-code="{{ $expert->uuid }}">
                                            <i class="icon-feather-check-circle"></i> Approve
                                        </button>
                                    </td>
                                    <td>
                                        <button class="decline btn btn-danger" style="color: #ffffff;" href="#" data-code="{{ $expert->uuid }}">
                                            <i class="icon-material-outline-highlight-off"></i> Decline
                                        </button>
                                    </td>
                                    <td>
                                        <a href="/users/freelancers/pending-approval/{{ $expert->uuid }}"  class="btn button" style="color: #fff; background: #17a2b8; padding: 5px 15px;">
                                            View
                                        </a>
                                    </td>
                                </tr>
